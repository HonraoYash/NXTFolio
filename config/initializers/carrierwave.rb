...
CarrierWave.configure do |config|
  ...
  config.fog_credentials = {
    ...
    :aws_access_key_id      => ENV.fetch("AWS_ACCESS_KEY", nil),            # required
    :aws_secret_access_key  => ENV.fetch("AWS_SECRET_KEY", nil),     # required
    :region                 => ENV.fetch("AWS_REGION", nil)                        # optional, defaults to 'us-east-1'
  }
  config.fog_directory  = ENV.fetch("AWS_BUCKET", nil)                       # required
