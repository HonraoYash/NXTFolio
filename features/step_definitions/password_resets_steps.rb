...
def extract_password_reset_token_from_email(_email_address)
  ...
  match_data = reset_link.match(%r{password/reset/edit\?token=(\w+)})
...
And(/^an email should be sent to "(.*?)" with a password reset link$/) do |_email|
