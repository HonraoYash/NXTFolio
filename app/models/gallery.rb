...
class Gallery < ApplicationRecord
  ...
  def get_similar_galleries
    ...
    all_galleries.each do |gallery|
      next unless gallery.GeneralInfo_id != self.GeneralInfo_id

      my_tags = tags.pluck(:body)
      other_tags = gallery.tags.pluck(:body)
      shared_tags = my_tags.intersection(other_tags)
      all_tags = my_tags + other_tags
      similarity = shared_tags.length.to_f / all_tags.length
      similar_galleries[gallery] = similarity
    end
    similar_galleries.reject! { |_key, val| val.nan? }
    Hash[similar_galleries.sort_by { |_key, val| val }.reverse]
  end
end
