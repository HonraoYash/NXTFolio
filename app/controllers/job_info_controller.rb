...
class JobInfoController < ApplicationController
  ...
  def post_job
    ...
    if @job_info.save
      ...
      filtered_users = GeneralInfo.filterBy(@job_info.country, @job_info.state, @job_info.profession,
                                            @job_info.city)
  ...
  def job_info_params
    ...
    params.require(:job_info).permit(:title, :description, :category, :profession, :country, :state, :city, :type,
                                     :low_salary, :high_salary, :userKey)
  ...
  def filter_words(string)
    ...
    string.split(/\W+/).each do |word|
      res.add(word.downcase.gsub(/\W+/, '')) unless stopwords_list.include?(word.downcase.gsub(/\W+/, ''))
    end
    res
  end
  ...
  def search
    ...
    @keyword = @params_args[:Keyword]
    @keyword = @keyword.downcase unless @keyword.nil?
    ...
    if @keyword.present?
      @filtered_users.each do |user|
        ...
        text_info += filter_words(user.title) unless user.title.nil?

        text_info += filter_words(user.description) unless user.description.nil?

        @keyword_set = filter_words(@keyword)

        text_info.each do |word|
          @keyword_set.each do |keyword|
            @final_users << user if word.include?(keyword)
          end
        end
      end
    else
