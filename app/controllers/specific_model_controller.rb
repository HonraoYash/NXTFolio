...
class SpecificModelController < ApplicationController
  ...
  def create
    ...
    @genre_str = ''
    unless params[:specific_model][:allgenres].nil?
      (0...params[:specific_model][:allgenres].size).each do |index|
        @genre_str = "#{@genre_str}#{params[:specific_model][:allgenres][index]},"
      end
    end
    ...
    return unless @specific_model.save!
  ...
  def update
    ...
    return unless @specific_model.update(specific_model_param)
  ...
  def specific_model_param
    params.require(:specific_model).permit(:height_feet, :height_inches, :bust, :waist, :hips, :cups, :shoe_size,
                                           :dress_size, :hair_color, :eye_color, :ethnicity, :skin_color, :shoot_nudes, :tattoos, :piercings, :experience, :genre, { allgenres: [] })
  end
