...
class SpecificDesignerController < ApplicationController
  ...
  def create
    ...
    unless params[:specific_designer][:allgenres].nil?
      (0...params[:specific_designer][:allgenres].size).each do |index|
        @genre_str = "#{@genre_str}#{params[:specific_designer][:allgenres][index]},"
      end
    end
    ...
    return unless @specific_designer.save
  ...
  def update
    ...
    return unless @specific_designer.update(specific_designer_param)
