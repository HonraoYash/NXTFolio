...
RSpec.describe ReviewsController, type: :controller do
  ...
  describe 'PUT#update' do
    it 'should update the review' do
      Review.create(rating: 5, comments: 'Test review', user_id: @general_info.id,
                    general_info_id: @general_info.id, gallery_id: @gallery.id)
      put :update,
          params: { id: Review.find_by_gallery_id(@gallery.id), gallery_id: @gallery.id,
                    review: { rating: 4, comments: 'Test review' } }
      expect(response).to redirect_to show_profile_show_profile_path(user_key: @general_info.userKey)
    end
    ...
    it 'should not update the review' do
      Review.create(rating: 5, comments: 'Test review', user_id: @general_info.id,
                    general_info_id: @general_info.id, gallery_id: @gallery.id)
      put :update,
          params: { id: Review.find_by_gallery_id(@gallery.id), gallery_id: @gallery.id, review: { rating: nil } }
